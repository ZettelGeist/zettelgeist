...
def get_argparse():
    ...
    parser.add_argument('--name', nargs='+', help="order of components, e.g. id, or timestamp")
    parser.add_argument('--name-dir', type=str, help="folder where to write file generated by --name (will NOT be created, if it does not exist)", default=".")
...
def main():
    ...
    if args.in_place:
        ...
        if args.file == args.save:
            print("Use --in-place instead of --save if you want to replace input file (specified with --file)")
            sys.exit(1)
